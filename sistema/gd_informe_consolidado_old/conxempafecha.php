...
<table width="593" border="0" cellpadding="0" cellspacing="0"   id="tabla_opciones"   >
		 ...
		 <tr id="fila_op_titulo"  >
		  ...
		  <tr id="fila_op_titulo"  >
		  <th colspan="3" style="background:none;"  align="center" ><em style="font-size:14px;">Hasta  el <?php echo $filtro_fecha; ?></em></th>
		  </tr>
		   <tr id="fila_op_titulo"  >
		  ...
		  <th width="133" >DOCUMENTOS CON ANEXOS</th>
			...
			<tbody>
		<?php 
		$sql="  select nombre, (select count(*) from gd_documento where gd_empresa.codigo=empresa and gd_empresa.borrado='N' and gd_documento.borrado='N' ".$filtro_tipo_doc." and cast(fecha_registro as date) <= cast('".$filtro_fecha."' as date) ) as cantidad_doc ,
...
from gd_empresa where borrado='N' and codigo in(select cod from sis_accesos_rol_usuario ('".$_SESSION["gd_usuario"]."','gd_empresa_inf','')) ".$filtro."order by  nombre ".$limite_reg;
		...
		<td width="133" align="center"  ><?php echo $fila["cantidad_doc"]; ?></td>
		<td width="133" align="center"  ><?php echo $fila["cantidad_con_anexo"]; ?></td>
		</tr>
		<?php }//for($i=1;$i<=20;$i++){ ?>
		</tbody>
</table>	
