...
if(!isset($_POST["txt_ubicaion"])){
		...
		where co.borrado='N'  and  co.contenedor_padre='".$_SESSION["gd_directorio_ini"]."' order by nombre";
		echo $sql;
		$opcion="gd_bodega";
	...
	if($txt_ubicacion==''){
			//$contenido="zona";
				...
				$sql="Select co.codigo,co.nombre,tp.imagen,co.tipo  from gd_contenedor co inner join gd_tipo_contenedor tp on co.tipo=tp.codigo
				where  co.estado='A' and co.contenedor_padre='".$_SESSION["gd_directorio_ini"]."' order by nombre";
				//echo $sql;
			...
			if($txt_ubicacion=="contenedor" ){
				...
				$contenedor=explode(";",$campos[(count($campos)-1)]);//gd_bodega
				$sql="Select co.codigo,co.nombre,tp.imagen,co.tipo  from gd_contenedor co inner join gd_tipo_contenedor tp on co.tipo=tp.codigo
				where  co.borrado='N'  and co.contenedor_padre='".$contenedor[1]."' order by nombre";
				//echo $sql;
				$opcion="gd_contenedor";
				$txt_title="Directorio/";
				$contenido="contenedor";
			...
			if($txt_ubicacion=="contenedor" and $existe_ext!==false){
				...
				(string_to_array((string_to_array(datos_documento,'|'))[1],';'))[2] as nombre , 'domuento16.png' as imagen, 'DO' as tipo from gd_documento 	
...
<div id="contenedor_ori">
	<div id="columnas_ori">
		<?php $res=pg_query($conn,$sql);
		$f=0;
		while ($reg=pg_fetch_array($res)) {$f++;?>
		<div id="objeto_ori<?php echo $f; ?>" class="unidad">
			<input type="checkbox" id="item_ori<?php echo $f; ?>" name="item_ori<?php echo $f; ?>" value="1" />
			<label  for="item_ori<?php echo $f; ?>"><span></span></label>
			<input type="hidden" name="codigo_ori[]" id="codigo_ori[]" value="<?php echo $reg["codigo"].";".$reg["tipo"]; ?>"/>
			<input type="hidden" name="tipo_ori[]" id="tipo_ori[]" value="<?php echo $reg["tipo"]; ?>"/>
