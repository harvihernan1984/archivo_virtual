...
if(!isset($_POST["txt_ubicaion"])){
		$sql="Select co.codigo,co.nombre,tp.imagen,co.tipo,tp.borrable  from gd_contenedor co inner join gd_tipo_contenedor tp on co.tipo=tp.codigo where  
		co.borrado='N' and co.contenedor_padre='-1' order by nombre";
		//echo $sql;
		$opcion="gd_dir_general";
		$txt_title='/';
	...
	if($txt_ubicacion==''){
			//$contenido="zona";
				...
				$sql="Select co.codigo,co.nombre,tp.imagen,co.tipo,tp.borrable  from gd_contenedor co inner join gd_tipo_contenedor tp on co.tipo=tp.codigo
				where  co.borrado='N' and  co.contenedor_padre='-1'  ".$filtro_buscar." order by co.nombre";
				//echo $sql;
				$opcion="gd_dir_general";
				$txt_title='/';
				$contenido="contenedor";
			...
			if($txt_ubicacion=="contenedor" and $existe_ext!==false){
				...
				(string_to_array((string_to_array(datos_documento,'|'))[1],';'))[2] from 1 for 50) as nombre , 'domuento16.png' as imagen, tipo, 'S' as borrable from gd_documento 	
				where  borrado='N' and contenedor='".$contenedor[1]."'    ".$filtro_buscar."  order by nombre";
				//echo $sql;
				$opcion="gd_ver_documento";
...
<div id="columnas">
	...
	<?php  if($opcion=='gd_dir_general'){ ?> 
		onclick="abrir_contenido('<?php echo $contenido; ?>','<?php echo $reg["codigo"];?>','<?php echo $reg["nombre"];?>','<?php echo $reg["tipo"];?>')" >
	<?php }?>
	<?php  if($opcion=='gd_ver_documento'){ ?>onclick="editar_contenido('<?php echo $opcion; ?>','<?php echo $reg["codigo"];?>')"> <?php }?>
	<div align="center"><img id="img_p" src="img/botones/<?php echo $reg["imagen"];?>" />  <p><?php echo $reg["nombre"];?></p></div>
   	</a>
   	</div>
...
<?php $res=pg_query($conn,$sql);
			...
			while ($reg=pg_fetch_array($res)) {?>
				<div class='icono_listado'    >
					...
					<img id="img_obj" src='img/botones/<?php echo $reg["imagen"];?>' width='20' height='20' ><?php echo $reg["nombre"];?>
					</a>
				</div>
<?php } //FIN while ($reg=pg_fetch_array($res))?>
</div>
