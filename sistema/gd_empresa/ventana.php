...
<script>
	function res_busca_persona(id){
		...
		$("#accion_windows").attr('value','selec_persona'); 
		$.ajax({ 
				type: 'POST',
				async: false, 
				data: $('#form_windows').serialize(),
				url: "datos_windows.php?op=persona&codigo="+id,  
	...
	function res_busca_directorio(id){
		$("#gif_cargando").css('visibility','visible');
		$("#accion_windows").attr('value','selec_directorio'); 
		$.ajax({ 
				type: 'POST',
				async: false, 
				data: $('#form_windows').serialize(),
				url: "datos_windows.php?op=directorio&codigo="+id,  
				success: function(data) {  
					$('#sis_resultado').html(data);
					cerrar_busqueda(); 
					$("#gif_cargando").css('visibility','hidden');
...
<input  type="hidden" id="ancho_windows" name="ancho_windows" value="550"/>
		...
		<input  type="hidden" id="filtro_windows" name="filtro_windows"   />
