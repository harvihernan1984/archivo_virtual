...
function borra_empresa(id){
	...
	$.messager.confirm('Mensaje del sistema', 'Esta seguro de borrar la Empresa... ?', function(r){
		if (r){
			$("#accion_contenido").attr('value','borrar');
        	$("#evento_contenido").attr('value','DEL');
			$.ajax({ 
					type: 'POST',
					async: true, 
...
<div align="left" id="encabezad" >
		<div id="toolbar" class="datagrid-toolbar" style=" position:relative; ">
        	<?php if(f_valida_acceso_rol('gd_empresa',$conn)==true){ ?>
        	...
        	<?php } //FIN if(f_valida_acceso_rol('gd_empresa',$conn)==true){ ?>
			...
			<input class="textbox" type="text" id="txt_buscar" name="txt_buscar" onKeyPress="BuscaEnter(event)"  />
             ...
             <select id="tipo_filtro" name="tipo_filtro">
             	<option selected="selected">nombre,ruc,direccion</option>
