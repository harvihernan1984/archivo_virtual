...
<table width="1000" border="0" cellpadding="0" cellspacing="0"   id="tabla_opciones"   >
		  ...
		  <th width="517"  >NOMBRE ROL</th>
		  </tr>
			</thead>
			<tbody>
		<?php 
		$sql="select * from sis_rol_usuario  where 
        codigo in(select cod from sis_accesos_rol_usuario ('".$_SESSION["gd_usuario"]."','sis_rol_usuario','')) ".$filtro." order by  nombre ".$limite_reg;
		//echo $sql;
		$num_reg=0;
		$res=pg_query( $conn,$sql);
