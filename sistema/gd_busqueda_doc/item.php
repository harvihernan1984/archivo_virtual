...
<tr height="20">
    ...
    <td  align="left"><input type="text" disabled="disabled" maxlength="300" size="50" value="<?php echo utf8_decode(utf8_decode($origen)); ?>"/></td>
    <td  align="left">&nbsp;</td>
    <td  align="left">&nbsp;</td>
  </tr>
  <tr height="20">
...
<div id="tb_listado_anexos" class="divgrid">
	<table id="tb_anexos" border="0" cellpadding="0" cellspacing="0"    width="100%">
		...
		<?php $sql="select *,dir_virtual || '/' || nombre_doc as ruta_doc from gd_documento_anexo where borrado='N' and  documento='".$codigo."'   order by  cast(codigo as integer) ";
		$num_reg=0;
		$res=pg_query( $conn,$sql);
		$a=0;
		...
		<tr>
				...
				<input  type="hidden"  name="codigo_anexo<?php echo $a;?>" id="codigo_anexo<?php echo $a;?>"  value="<?php echo $reg["codigo"];?>" />
				</td>
				<td>
				</td>
				<td>
				<a id="cmd_descargar_anexo" class="easyui-linkbutton l-btn l-btn-small l-btn-plain"  
				plain="true" href="javascript:void(0)"  target='_blank'  onclick="registrar_consulta('<?php echo $reg["ruta_doc"];?>','<?php echo $a;?>');">
