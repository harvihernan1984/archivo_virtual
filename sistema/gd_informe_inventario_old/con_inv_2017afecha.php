...
$filtro_x_tipo_doc="";
if($tipo_documento!='-1'){$filtro_x_tipo_doc=" and tipo='".$tipo_documento."'";}
$filtro=$filtro_x_tipo_doc.$filtro;
//FILTRO PARA EMPRESA
$filtro_x_empresa="";
if($empresa!='-1'){$filtro_x_empresa=" and empresa='".$empresa."'";}
$filtro=$filtro_x_empresa.$filtro;

$sql="SELECT (count(*) /  ".$_POST["limite_reg"]." ) + 1 as total_paginas , count(gd_doc.*) as total_doc,  
(select get_numero(valor) from sis_configuracion where codigo='NUM_EXP' and tipo='CONF') as limte_reg_conf FROM gd_documento where borrado='N' and gd_doc.empresa in(select cod from sis_accesos_rol_usuario ('".$_SESSION["gd_usuario"]."','gd_empresa_inf',''))
and cast(fecha_registro as date) <= cast('".$filtro_fecha."' as date) ".$filtro;
//echo $sql;
...
<table width="988" border="0" cellpadding="0" cellspacing="0"   id="tabla_opciones"   >
		 ...
		 <tr id="fila_op_titulo"  >
		  ...
		  <tr id="fila_op_titulo"  >
		  <th colspan="15" style="background:none;"  align="center" ><em style="font-size:14px;">Hasta  el <?php echo $filtro_fecha; ?></em></th>
		  </tr>
		   <tr id="fila_op_titulo"  >
...
'' as apertura,
'' as cierre,
coalesce((select sum(fojas) from gd_documento_anexo where documento=gd_doc.codigo),0) as fojas,
...
'' as estado,
'' as legibilidad,
case when (select count(*) from gd_documento_anexo where documento=gd_doc.codigo) > 0 then 'SI' else 'NO' end as digitalizacion,
get_directorio_documento(gd_doc.contenedor) as ubicacion,
'' as responsable,
'' as observacion
from gd_documento gd_doc where gd_doc.borrado='N' and gd_doc.empresa in(select cod from sis_accesos_rol_usuario ('".$_SESSION["gd_usuario"]."','gd_empresa_inf',''))  and cast(fecha_registro as date) <= cast('".$filtro_fecha."' as date)  ".$filtro." order by  cast(contenedor as integer),cast(gd_doc.codigo as integer) ".$limite_reg;
		//echo $sql;
		$num_reg=0;
		$res=pg_query( $conn,$sql);
		...
		<td width="28" align="right"  ><?php echo $fila["fojas"]; ?></td>
